...
const Register = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!validate()) return showToast(t("registrationPage.fixErrors"), "error");
    ...
    try {
      ...
      if (!result.error) {
        showToast(t("registrationPage.emailVerificationSent"), "success");
      } else {
        showToast(result.error || t("registrationPage.error"), "error");
      }
    } catch {
            ...
            <div className="auth-container">
              <div className="auth-section">
                <div className="form-container login">
                  <h2 className="form-title">{t("registrationPage.createAccount")}</h2>
                  <div className="auth-info-content login">
                    <h2>{t("registrationPage.joinUs")}</h2>
                    <p>{t("registrationPage.fillDetails")}</p>
                  </div>

                  <form id="registration-form" autoComplete="off" onSubmit={handleSubmit}>
                    <div className={`input-box ${errorClass("firstName")}`}>
                      <input type="text" id="fname" required value={formData.fname} onChange={handleChange} />
                      <label htmlFor="fname">{t("registrationPage.firstName")}</label>
                      {errors.firstName && <p className="input-error">{errors.firstName}</p>}
                    </div>

                    <div className={`input-box ${errorClass("lastName")}`}>
                      <input type="text" id="lname" required value={formData.lname} onChange={handleChange} />
                      <label htmlFor="lname">{t("registrationPage.lastName")}</label>
                    ...
                    <div className={`input-box position-relative ${errorClass("password")}`}>
                      <input type={showPassword ? "text" : "password"} id="password" required value={formData.password} onChange={handleChange} />
                      <label htmlFor="password">{t("registrationPage.password")}</label>
                      <i className={`bx ${showPassword ? "bx-show" : "bx-hide"} password-toggle`}
                    ...
                    <div className={`input-box position-relative ${errorClass("confirmPassword")}`}>
                      <input type={showConfirmPassword ? "text" : "password"} id="confirmPassword" required value={formData.confirmPassword} onChange={handleChange} />
                      <label htmlFor="confirmPassword">{t("registrationPage.confirmPassword")}</label>
                      <i className={`bx ${showConfirmPassword ? "bx-show" : "bx-hide"} password-toggle`}
                    ...
                    <div className="input-box">
                      {!loading ? (
                        <button id="register-btn" className="btn w-100" type="submit">
                          {t("registrationPage.registerBtn")}
                        </button>
                      ) : (
                        <button className="btn w-100" type="button" disabled>
                          <span className="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span>
                          <span className="ms-2">{t("registrationPage.registeringBtn")}</span>
                        </button>
                      )}
                    </div>
                    ...
                    <div className="auth-link mt-3">
                      <p>
                        {t("registrationPage.haveAccount")} <NavLink className="signUp-link" to="/login">{t("registrationPage.loginLink")}</NavLink>
                      </p>
                    </div>
                  </form>
                </div>
              ...
              {/* Toast Notification */}
              {toast.message && (
                <div className={`toast-notification ${toast.type === "success" ? "bg-green-500" : "bg-red-500"} text-white p-3 rounded mt-3`} role="alert">
                  {toast.message}
                </div>
              )}
            </div>
