...
const Register = () => {
  ...
  const validate = () => {
    const newErrors = {};
    if (!formData.fname.trim()) newErrors.firstName =` {t('loginPage.firstNameRequired')}`;
    else if (!formData.fname.trim().length < 3) newErrors.firstName ="First name must be at least 3 characters long";
    if (!formData.lname.trim()) newErrors.lastName =` {t('loginPage.lastNameRequired')}`;
    else if (!formData.lname.trim().length < 3) newErrors.lastName ="Last name must be at least 3 characters long";
    if (!formData.email.trim()) newErrors.email =` {t('loginPage.emailRequired')}`;
    if (!formData.password.trim()) newErrors.password =` {t('loginPage.passwordRequired')}`;
    else if (formData.password.trim().length < 8) newErrors.password = "Password must be at least 5 characters long.";
    if (!formData.confirmPassword.trim()) newErrors.confirmPassword =` {t('loginPage.passwordRequired')}`;
    else if (formData.confirmPassword.trim().length < 8) newErrors.confirmPassword = "Password must be at least 5 characters long.";
    setErrors(newErrors);
  ...
  const handleSubmit = async (e) => {
    ...
    if (formData.password !== formData.confirmPassword) {
      alert(t("registrationPage.passwordsMismatch"));
      return;
    }
    setLoading(true);
    try {
